...
public class TestQueryEquals {
	...
	public void testQueryEquals1() {
		...
		bp.add(new Triple(var_x, NodeFactory.createURI(BASE + "p"), var_v));
		Op op3 = new OpBGP(bp);
		Expr expr = new E_Equals(new ExprVar(var_v),
				NodeValue.makeNodeInteger(1));
	...
	public void testQueryEquals2() {
		Query q1 = QueryFactory.read("src/test/resources/query-1.rq");
		Query q3 = QueryFactory.read("src/test/resources/query-3.rq"); // the
																		// only
																		// difference
																		// is
																		// the
		...
		it1 = ItemTransformer
				.transform(new RenameVariablesItemTransform(), it1);
		Item it3 = SSE.parse(op3.toString());
		it3 = ItemTransformer
				.transform(new RenameVariablesItemTransform(), it3);
