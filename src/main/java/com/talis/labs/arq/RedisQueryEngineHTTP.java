...
public class RedisQueryEngineHTTP extends QueryEngineHTTP implements Closeable {
	...
	public ResultSet execSelect() {
		...
		if (value != null) {
			rs = ResultSetFactory.makeRewindable(ResultSetFactory
					.fromXML(value));
		} else {
	...
	public void invalidate(String service) {
		Set<String> keys = client
				.smembers(Integer.toString(service.hashCode()));
		for (String key : keys) {
			client.del(key);
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < 10000; i++) {
			...
			RedisQueryEngineHTTP qexec = new RedisQueryEngineHTTP(serviceURI,
					"SELECT * { ?s ?p ?o } LIMIT 1000");
			if (i % 1000 == 0) {
