...
public class CachedQueryExecutionBase extends QueryExecutionBase {
	...
	private static Cache<Pair<Dataset, Query>, Object> cache = CacheFactory
			.createCacheUnbounded();

	static {
		cache.setDropHandler(new ActionKeyValue<Pair<Dataset, Query>, Object>() {
			@Override
			public void apply(Pair<Dataset, Query> key, Object value) {
			}
		});
	}

	public CachedQueryExecutionBase(Query query, Dataset dataset,
	...
	public ResultSet execSelect() {
		...
		rs = (ResultSetRewindable) ResultSetFactory.makeRewindable(super
				.execSelect());
		synchronized (cache) {
			cache.put(key, rs);
		}
	...
	public static void main(String[] args) {
		...
		Dataset dataset = DatasetFactory.create(
				"src/test/resources/dataset/dft.n3", uriList);
		for (int i = 0; i < 100000; i++) {
			Timer timerQuery = new Timer();
			timerQuery.startTimer();
			Query query = QueryFactory.create("SELECT * { ?s_"
					+ random.nextInt(100) + " ?p ?o } LIMIT 100");
			Context context = ARQ.getContext().copy();
			QueryEngineFactory qeFactory = QueryEngineRegistry.get().find(
					query, dataset.asDatasetGraph(), context);
			CachedQueryExecutionBase qexec = new CachedQueryExecutionBase(
					query, dataset, context, qeFactory);
			if (i % 1000 == 0) {
